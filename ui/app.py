...
def format_metrics(metrics_data: dict) -> str:
    ...
    output.append(f"Total Tool Calls: {metrics['total']['tool_calls']}")
    output.append(f"Total Iterations: {metrics['total']['iterations']}")
    ...
    for agent in ['planner', 'developer', 'tester']:
        ...
        if agent_metrics.get('api_calls', 0) > 0:
            output.append(f"\n🤖 {agent.upper()} Agent:")
            output.append(f"   API Calls: {agent_metrics.get('api_calls', 0)}")
            output.append(f"   Tool Calls: {agent_metrics.get('tool_calls', 0)}")
            output.append(f"   Iterations: {agent_metrics.get('iterations', 0)}")
